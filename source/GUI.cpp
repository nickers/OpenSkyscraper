...
GUI::GUI(std::string name, GUIManager * manager)
{
	...
	context = Rocket::Core::CreateContext(name.c_str(), Rocket::Core::Vector2i(manager->window->getSize().x, manager->window->getSize().y));
...
bool GUI::handleEvent(sf::Event & event)
{
	switch (event.type) {
		case sf::Event::Resized:
			this->context->SetDimensions(Rocket::Core::Vector2i(manager->window->getSize().x, manager->window->getSize().y));
			return true;
...
void GUI::draw()
{
	...
	glPushMatrix();
	glLoadIdentity();
	glOrtho(0, manager->window->getSize().x, manager->window->getSize().y, 0, -1, 1);
